...
import type { CreateGenerationPayload, GenerationRecord, PaginatedResponse } from "@/lib/api/types";
...
export async function getGeneration(id: string, opts?: ServiceOptions): Promise<GenerationRecord> {
  return api.get<GenerationRecord>(`${API_V1}/generations/${encodeURIComponent(id)}`, {
    ...opts,
    auth: opts?.auth ?? true
  });
}
...
export async function listHistory(params: ListHistoryParams = {}, opts?: ServiceOptions): Promise<PaginatedResponse<GenerationRecord>> {
...
export async function deleteGeneration(id: string, opts?: ServiceOptions): Promise<void> {
...
export async function duplicateGeneration(id: string, opts?: ServiceOptions): Promise<GenerationRecord> {
...
export async function regenerate(id: string, opts?: ServiceOptions): Promise<GenerationRecord> {
  return api.post<GenerationRecord>(
